-- ============================================================================
--  MIGRATION DEFINITIVE NARUSTREAM (AVEC FIX RLS RÉCURSION)
-- ============================================================================

-- ============================================================================
--  1. AJOUT DES COLONNES MANQUANTES A LA TABLE MOVIES
-- ============================================================================
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movies' AND column_name = 'age_rating') THEN
    ALTER TABLE public.movies ADD COLUMN age_rating text DEFAULT '10+';
  END IF;
  -- Fix: Change age_rating to text type if it's currently integer
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movies' AND column_name = 'age_rating' AND data_type = 'integer') THEN
    ALTER TABLE public.movies ALTER COLUMN age_rating TYPE TEXT USING age_rating::TEXT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movies' AND column_name = 'platform') THEN
    ALTER TABLE public.movies ADD COLUMN platform text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'movies' AND column_name = 'status') THEN
    ALTER TABLE public.movies ADD COLUMN status text DEFAULT 'sortie';
  END IF;
END $$;

-- ============================================================================
--  2. AJOUT DES COLONNES MANQUANTES A LA TABLE STREAMS
-- ============================================================================
DO $$ BEGIN
...
-- ============================================================================
--  3. AJOUT DES COLONNES MANQUANTES A LA TABLE EPISODES
-- ============================================================================
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'episodes' AND column_name = 'status') THEN
    ALTER TABLE public.episodes ADD COLUMN status text DEFAULT 'sortie';
  END IF;
END $$;

-- ============================================================================
--  3.5. AJOUT DES COLONNES MANQUANTES A LA TABLE SAGAS
-- ============================================================================
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'sagas' AND column_name = 'description') THEN
    ALTER TABLE public.sagas ADD COLUMN description text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'sagas' AND column_name = 'poster_url') THEN
    ALTER TABLE public.sagas ADD COLUMN poster_url text;
  END IF;
END $$;

-- ============================================================================
--  4. TABLE PROFILES (AMÉLIORÉE)
-- ============================================================================
...
-- ============================================================================
--  9. FONCTION POUR CHECKER SI UTILISATEUR EST ADMIN (évite récursion)
-- ============================================================================
...
-- ============================================================================
--  10. DEFINIR TOUTES LES POLITIQUES RLS (FIXÉES)
-- ============================================================================
...
CREATE POLICY "Admins can manage movies" ON public.movies FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can manage episode_streams" ON public.episode_streams FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can view all lists" ON public.user_lists FOR SELECT USING (public.is_admin());
...
CREATE POLICY "Admins can view all watch history" ON public.watch_history FOR SELECT USING (public.is_admin());
...
CREATE POLICY "Admins can view all stream logs" ON public.stream_logs FOR SELECT USING (public.is_admin());
...
CREATE POLICY "Admins can manage content_requests" ON public.content_requests FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can manage sagas" ON public.sagas FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can manage saga_movies" ON public.saga_movies FOR ALL USING (public.is_admin());
...
CREATE POLICY "Admins can manage site_config" ON public.site_config FOR ALL USING (public.is_admin());
...
-- POLITIQUES POUR CONTACT MESSAGES
DROP POLICY IF EXISTS "Allow public insert on contact messages" ON public.contact_messages;
CREATE POLICY "Allow public insert on contact messages" ON public.contact_messages FOR INSERT WITH CHECK (true);
...
-- ============================================================================
--  11. CREER LES INDEX POUR LES PERFORMANCES
-- ============================================================================
